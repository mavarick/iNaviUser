...
def upload_avatar(request):
    ...
    if UploadedImage.objects.filter(username=get_uid(request)).exists():
        UploadedImage.objects.filter(username=get_uid(request)).update(image=new_name)
    else:
        UploadedImage.objects.create(username=get_uid(request), image=new_name)
...
def crop_avatar(request):
    try:
        upim = UploadedImage.objects.get(username=get_uid(request))
    ...
    avatar_crop_done.send(sender=None, username=get_uid(request), avatar_name=avatar_name)
